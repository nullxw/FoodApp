...
@implementation Chracter
@synthesize allChracter;

-(id)init
{
    if(self = [super init])
    {
        //self.allChracter = [[NSArray alloc] initWithObjects:@"A安",@"B比",@"C查", nil];
        self.allChracter = [[NSArray alloc] initWithObjects:
...
-(NSString *)getFirstChracter:(NSString *)strText
{
    ...
    NSUInteger length = strText.length;
//    NSUInteger length = 1;
    if(length == 0)
    {
        return strText ; 
    ...
    for(int i = 0; i < 1; i ++)
    {
        ...
        else if((int)character >= 19968 && (int)character <= 40869)
        {
            
            NSRange range = {i,1};
            NSRange firstRange = {0,1};
            for(NSString *str in allChracter)
            {
                NSRange resultRange = [str rangeOfString:[strText substringWithRange:range]];
                if(resultRange.length > 0)
                {
                    [result appendFormat:[str substringWithRange:firstRange]];
                    break;
                }
            }
        }
