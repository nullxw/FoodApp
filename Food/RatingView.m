...
-(void)displayRating:(float)rating {
    ...
    if(rating<=3)
    {
        fullySelectedImage=[UIImage imageNamed:@"Public_starLow.png"];
    }
    else
    {
        fullySelectedImage=[UIImage imageNamed:@"Public_starFull.png"];
    }
	...
	starRating = rating;
	lastRating = rating;
	[viewDelegate ratingChanged:rating andRatongView:self];
}
...
-(void) touchesMoved: (NSSet *)touches withEvent: (UIEvent *)event
{
	...
	int newRating = (float) (pt.x / width) + 1;
